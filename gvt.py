...
class res11(nn.Module):
    def __init__(self):
        super().__init__()
        self.conv1 = nn.Conv2d(64,128,2,2,0)
        self.bn = nn.LayerNorm(128)

    def forward(self,x,y):
        b = x.shape[0]
        x = x.transpose(1,2).reshape(b,64,56,56)
        x = self.conv1(x)
        x = x.reshape(b,128,784).transpose(1,2)
        x = x + y
        x = self.bn(x)
        return x


class res33(nn.Module):
    def __init__(self):
        super().__init__()
        self.conv1 = nn.Conv2d(256,512,2, 2)
        self.bn = nn.LayerNorm(512)

    def forward(self, x, y):
        b = x.shape[0]
        x = x.transpose(1, 2).reshape(b, 256,14,14)
        x = self.conv1(x)  # [b,512,7,7]
        x = x.reshape(b, 512, 49).transpose(1, 2)
        x = x + y
        x = self.bn(x)
        return x
class Pool(nn.Module):
    def __init__(self):
        ...
        self.conv2 = nn.Conv2d(in_channels=128,out_channels=256,kernel_size=1,stride=1)
        self.bn3 = nn.LayerNorm(512)
        self.conv3 = nn.Conv2d(in_channels=256,out_channels=512,kernel_size=1,stride=1)

    def forward(self,i,x,x1):
        b = x.shape[0]
        if i == 0:
            x2 = 0
            x3 = x1 + x2
        if i == 1:
            x = self.conv1(x)
            x = self.pool(x)
            x2 = x.reshape(b,128,-1).transpose(1,2)
            x3 = x1 + x2
            x3 = self.bn1(x3)
        if i == 2:
            x = self.conv2(x)
            x = self.pool(x)
            x2 = x.reshape(b, 256, -1).transpose(1, 2)
        ...
        if i == 3:
            x = self.conv3(x)
            x = self.pool(x)
            x2 = x.reshape(b, 512, -1).transpose(1, 2)
            x3 = x1 + x2
            x3 = self.bn3(x3)
...
class PyramidVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        self.a = torch.nn.Parameter(torch.FloatTensor(1), requires_grad=True)  # 设置一个可学习的标量
        self.a.data.fill_(0.1)  # 初始化
        self.b = torch.nn.Parameter(torch.FloatTensor(1), requires_grad=True)  # 设置一个可学习的标量
        self.b.data.fill_(0.2)  # 初始化
        self.c = torch.nn.Parameter(torch.FloatTensor(1), requires_grad=True)  # 设置一个可学习的标量
        self.c.data.fill_(0.3)  # 初始化
        self.d = torch.nn.Parameter(torch.FloatTensor(1), requires_grad=True)  # 设置一个可学习的标量
        self.d.data.fill_(0.4)  # 初始化
    ...
    def forward(self, x):
        ...
        x3 = self.head(x3)
        x4 = self.head(x4)
        x = self.a*x1 + self.b*x2 + self.c*x3 + self.d*x4
        return x
...
class CPVTV2(PyramidVisionTransformer):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        self.line1 = nn.Linear(in_features=64,out_features=512,bias=True)
        self.line2 = nn.Linear(in_features=128, out_features=512, bias=True)
        self.line3 = nn.Linear(in_features=256, out_features=512, bias=True)
        self.pool = Pool()
        self.res1 = res11()
        self.res3 = res33()
    ...
    def forward_features(self, x):
        ...
        for i in range(len(self.depths)):
            ...
            x1, (H, W) = self.patch_embeds[i](x)
            x2 = self.pool(i, x, x1)
            x = self.pos_drops[i](x2)
...
@register_model
def afvf(pretrained=False, **kwargs):
    model = ALTGVT(
        patch_size=4, embed_dims=[64, 128, 256, 512], num_heads=[2, 4, 8, 16], mlp_ratios=[4, 4, 4, 4], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[4, 4, 4, 4], wss=[7, 7, 7, 7], sr_ratios=[8, 4, 2, 1],
        **kwargs)
    model.default_cfg = _cfg()
    return model